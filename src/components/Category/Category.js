...
export class Category extends PureComponent {
  ...
  reload () {
    const { navigation } = this.props;
    const videos = navigation.state.params.category.videos.map(video => ({...video, name: video.video.split('/').pop()}));
      ...
      if (!video.downloaded) {
        RNFS.downloadFile({
          ...
          this.setState(prevState => {
            ...
            return {downloadedVideos: prevState.downloadedVideos + 1, initialAmount:prevState.initialAmount-1, videos: changingVideos};
          });
        });
      } else {  
        this.setState(prevState => ({downloadedVideos: prevState.downloadedVideos, initialAmount:prevState.initialAmount-1}));
      }
    })
  }
  ...
  render() {
    ...
    return (
      <View style={styles.full} onLayout={this._onLayout}>
        ...
        >
        {/* <Text style={styles.headerText}>{amount} VIDEOS DESCARGADOS  DE {videosAmount}</Text> */}
        <ScrollView >
          ...
          />{
            ...
            Alert.alert(
              'DESCARGA VIDEOS',
              `VAS A DESCARGAR ${this.state.initialAmount} DE ${videosAmount} VIDEOS. ESTA ACCIÓN PUEDE DEMORAR UN POCO.`,
        ...
        {this.state.showBar && 
          <View>
           <Progress.Bar color='green' width={null} progress={this.state.downloadedVideos / videosAmount}/>
           <Text style={styles.downloadText}>{`${this.state.downloadedVideos} de ${videosAmount}`}</Text>
         </View>
        }</ImageBackground>
      </View>
    );
